...
本课题面向水下无线传感器网络（UWSN）节能路由问题，旨在引入强化学习机制，设计一种能够协同优化分簇结构与路由决策的智能化算法——EC-QL（Energy-efficient Clustering Q-Learning）。具体研究内容涵盖以下四个相互支撑的方面。

1. 设计基于 Q-learning 的自适应分簇算法，将簇头选举过程建模为多智能体强化学习问题。每个传感器节点作为独立智能体，以其剩余能量为状态，以“成为簇头”或“保持为成员”为动作，通过奖励函数引导网络自主选出能量充裕、位置适中的节点担任簇头，克服传统 LEACH 协议随机选举的盲目性。

2. 构建基于 Q-learning 的自适应路由模型。在分簇完成后，以簇头为路由智能体，将多跳数据转发路径的规划建模为强化学习决策过程，综合考量下一跳节点的剩余能量、向 Sink 的前进距离与链路质量，通过 Q 表的在线迭代学习，逐步收敛至能耗均衡与传输效率兼顾的骨干路由策略。

3. 设计网络生命周期指标与综合奖励函数。定义以首节点死亡轮次（FND）为核心的网络寿命评价体系，并设计多维度综合奖励函数——将能量保护、传输进度、链路质量、传播延迟与距离惩罚以动态加权形式融合，使两层 Q-learning 的底层优化目标与延长网络全生命周期的全局目标保持一致。

4. 最后是算法仿真验证与性能评估。以 MATLAB 为仿真平台，在统一的三维水声信道环境中将 EC-QL 与 LEACH、QELAR 两种基准协议进行全面对比，围绕网络寿命、端到端延迟、能耗均衡性与鲁棒性等关键指标系统评估算法性能。
